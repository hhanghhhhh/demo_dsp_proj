...
{
    Uint8 ret;				// ret����Ϊ�Ƿ����ӵı�־λ��ret=0  �����жϣ�ret=1���ӳɹ�
    if							
        (
    ...
    else							// ���Ŀ��IP��Portδ���ã����������
    {
        ...
        IINCHIP_WRITE( Sn_DPORT1(s), (Uint8)(port & 0x00ff));
				...
				IINCHIP_WRITE( Sn_CR(s) ,Sn_CR_CONNECT);						// Sn_CR��ֵ��Ϊ0x04����ִ��TCP������������

        while ( IINCHIP_READ(Sn_CR(s) ) ) ;									// MCU��ȡSn_CR(s)����ֵ

        while ( IINCHIP_READ(Sn_SR(s)) != SOCK_SYNSENT )		// ��ʱSn_SR(s)�Ĵ���Ӧ�ô���SOCK_SYNSENT�������ų����ڸ�״̬�ļ������
        {
            if(IINCHIP_READ(Sn_SR(s)) == SOCK_ESTABLISHED)	// Socket�����Ѿ���������������
            {
                break;
            }
            if (getSn_IR(s) & Sn_IR_TIMEOUT)								// ��ARPto��TCPto��ʱ���쳣
            {
                IINCHIP_WRITE(Sn_IR(s), (Sn_IR_TIMEOUT));   // ֪ͨMCU���жϣ������ж�
                ret = 0;																		// ret��0�������ж�
                break;
            }
        }
...
*/
Uint16 recv(SOCKET s, Uint8 * buf, Uint16 len)//16->32
{
   Uint16 ret=0;
   if ( len > 0 )
   {
...
Uint16 macraw_recv( Uint8 * buf, Uint16 len )
{
   ...
   if ( len > 0 )
   {
      ...
      if(data_len > 1514)
      {
         ...
         close_socket(sock_num); // Close the 0-th socket
         socket(sock_num, Sn_MR_MACRAW, dummyPort,mFlag);  // OPen the 0-th socket with MACRAW mode
         return 0;
      }
