...
InitMcbspa(void)
{
    ...
    McbspaRegs.PCR.bit.CLKXM = 1;   //SPI master
    McbspaRegs.PCR.bit.FSXM = 1;   //The FSX pin is an output pin.  SPI master
    McbspaRegs.SRGR2.bit.FSGM = 0;   //The transmitter drives a frame-synchronization pulse on the FSX pin every time data is
                                     //transferred from DXR1 to XSR1.
    McbspaRegs.PCR.bit.FSXP = 1;     //The FSX pin is active low.
    ...
    // LSPCLK--internal clk 01    10 external CLKR    11 external CLKX
    ...
    //Single-phase frame, 1 word/frame, 16bit   (Receive)
    //Single-phase frame, 1 word/frame, 16bit  (Transmit)
    McbspaRegs.XCR2.bit.XPHASE = 0;
    McbspaRegs.XCR1.bit.XFRLEN1 = 0;
    McbspaRegs.RCR2.bit.RPHASE = 0;
    McbspaRegs.RCR1.bit.RFRLEN1 = 0;
    InitMcbspa16bit();  // 16-bit word


    // CLKG frequency = LSPCLK/(CLKGDV+1)
    McbspaRegs.SRGR1.bit.CLKGDV = 3;        // Frame Width = 1 CLKG period, CLKGDV=4  37.5M/4 = 9.375M
...
InitMcbspb(void)
{
    ...
    McbspbRegs.PCR.bit.CLKXM = 1;   //SPI master
    McbspbRegs.PCR.bit.FSXM = 1;   //The FSX pin is an output pin.  SPI master
    ...
    // CLKG frequency = LSPCLK/(CLKGDV+1)
    McbspbRegs.SRGR1.bit.CLKGDV = 36;        // Frame Width = 1 CLKG period, CLKGDV=4  37.5M/4 = 9.375M
